...
**Despite DockerManager's modest capabilities, it is quite suitable for home use:**
...
Starting with `v2.0`, multiple selections in lists are supported when deleting/stopping images/containers. To select multiple list entries, press and hold `Ctrl` while choosing an action from the pop-up menu. This enables the "Pause lists..." mode (see window title), which allows you to select records in static mode. Additionally, you can use the `Esc` button to interrupt frozen Download/Upload processes when working with DockerHub.
